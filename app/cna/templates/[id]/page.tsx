...
export default function CnaTemplateDetailPage() {
  ...
  const fetchConnectedData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/cna/templates/${templateId}/connected-data`,
        {
          params: {
      ...
      showNotification({
        title: "Error",
        message: "No fue posible cargar la informacion consolidada CNA.",
        color: "red",
      });
    } finally {
      setLoading(false);
  ...
  const handleDownload = () => {
    ...
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL}/cna/templates/${templateId}/download-connected-data?email=${encodeURIComponent(
        session.user.email
      )}`,
      "_blank",
      "noopener,noreferrer"
  ...
  const handleDownloadFieldComparison = () => {
    ...
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL}/cna/templates/${templateId}/download-field-comparison?email=${encodeURIComponent(
        session.user.email
      )}`,
      "_blank",
      "noopener,noreferrer"
  ...
  return (
    <Container size="xl" py="xl">
      <Group justify="space-between" mb="md">
        <Title order={1}>
          {data ? `Datos CNA para: ${data.template.name}` : "Cargando plantilla CNA..."}
        </Title>
      </Group>
      ...
      {!data && !loading && (
        <Center>
          <Text c="dimmed">No se encontro informacion CNA para esta plantilla.</Text>
        </Center>
      )}
              ...
              >
                <ScrollArea style={{ height: "calc(100% - 32px)" }} scrollbarSize={8}>
                  <Table
                    striped
                    withTableBorder
                    style={{ minWidth: "100%", borderCollapse: "collapse", fontSize: "14px" }}
                  >
                    <Table.Thead
                      style={{
                        position: "sticky",
